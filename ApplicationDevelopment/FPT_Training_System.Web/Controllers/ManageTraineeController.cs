...
{
    public class ManageTraineeController : BaseController
    {
        ...
        public ManageTraineeController(
        ...
        public async Task<ActionResult> Index(CancellationToken cancellationToken, string searchString)
        {
            ...
            var searchtrainee = await (from u in _context.AspNetUsers
                                       join ur in _context.AspNetUserRoles on u.Id equals ur.UserId
                                       join r in _context.AspNetRoles on ur.RoleId equals r.Id
                                       where r.Name == "Trainee"
                                       select u).ToListAsync(cancellationToken);
            ...
            {
                searchtrainee = searchtrainee.Where(s => s.Fullname.Contains(searchString) || s.MainPpLanguage.Contains(searchString) || s.ToeicScore.ToString().Contains(searchString)).ToList();
            }

            ViewBag.user = searchtrainee;
        ...
        public ActionResult Create([Bind(Include = "Id,Fullname,Email,Password,Age,Dob,Education,MainPpLanguage,ToeicScore,ExperienceDetails,Department,Location,Contact,isActive")] AspNetUser aspNetUser)
        {
            if (ModelState.IsValid)
            {
                ...
                var insertedUserRoleId = new AspNetUserRole { RoleId = 4, UserId = aspNetUser.Id };
